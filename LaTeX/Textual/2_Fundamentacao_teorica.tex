...
\textcolor{red}{A tendência recente de emprego de robôs humanóides na execução de tarefas industriais, resultou no desenvolvimento de uma nova geração de modelos de inteligência artificial que integram três capacidades fundamentais em um único sistema para dar "inteligência geral" a estes robôs. Estes modelos são denominados VLA (\textit{Vision-Language-Action}) ou Visão-Linguagem-Ação.}
\abbrev{VLA}{Vision-Language-Action}

\textcolor{red}{\textbf{Visão:} O robô utiliza câmeras, LiDAR (\textit{Light Detection and Ranging}) e algoritmos de visão computacional para compreender o ambiente ao seu redor, identificando objetos e obstáculos.}
\abbrev{LiDAR}{Vision-Language-Action}

\textcolor{red}{\textbf{Linguagem:} O sistema processa instruções em linguagem natural (texto ou fala), permitindo que humanos deem comandos complexos como "pegue a caneca vermelha e coloque-a na mesa".}

\textcolor{red}{\textbf{Ação:} O modelo traduz a compreensão visual e as instruções verbais diretamente em comandos motores, controlando os movimentos físicos do robô de forma coordenada.}

\textcolor{red}{Ao contrário da robótica tradicional, os modelos VLA permitem que os robôs aprendam a realizar tarefas e se adaptem a novas situações de forma mais flexível.}

\textcolor{red}{Neste projeto de graduação serão abordados somente os modelos de planejamento de trajetórias baseados em códigos de programação, sem considerar os modelos VLA.}