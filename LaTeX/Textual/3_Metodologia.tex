...
\begin{itemize}
    \item \textbf{\textit{HTML}}: responsável pela estruturação dos elementos da interface;
    \item \textbf{\textit{CSS}}: utilizado para organização visual e disposição dos componentes na página;
    \item \textbf{\textit{JavaScript}}: empregado na implementação da lógica do sistema;
    \item \textbf{\textit{Babylon.js}}~\cite{babylonjs}: biblioteca gráfica baseada em \textit{WebGL} utilizada para renderização tridimensional e manipulação da cena;
    \item \textbf{\textit{Chart.js}}~\cite{chartjs}: biblioteca utilizada para geração dos gráficos de posição, velocidade e aceleração das juntas.
\end{itemize}
...
\begin{itemize}
    \item \texttt{simulador.html}: define a estrutura da interface e os elementos visuais;
    ...
    \item \texttt{trajectory\_math.js}: contém o cálculo dos coeficientes dos polinômios cúbicos, sua avaliação temporal e a sugestão das velocidades nos pontos intermediários.
\end{itemize}
...
\begin{enumerate}
    \item O usuário interage com a interface por meio de controles deslizantes ou comandos de execução de trajetória. Essa interação pode ser por meio da cinemática direta ou inversa;
    \item Caso o comando do usuário seja por meio da cinemática inversa, os valores das juntas são calculados;
    \item As variáveis articulares são atualizadas;
    \item A nova configuração do manipulador é aplicada à estrutura hierárquica da cena 3D;
    \item O posicionamento do efetuador final é recalculado e, quando aplicável, utilizado na atualização dos gráficos;
    \item O motor gráfico renderiza a cena atualizada em tempo real.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Imagens/3_Metodologia/flowchart.png}
    \caption{Fluxograma do funcionamento do programa }
    \label{fig:flowchart}
\end{figure}

%https://miro.com/app/board/uXjVHfj6woE=/
...
A cena tridimensional foi configurada utilizando um sistema de coordenadas de x, y e z, com o eixo $z$ orientado verticalmente, em concordância com a modelagem cinemática adotada.
...
Com base na geometria definida, foram estabelecidos os parâmetros de \textit{Denavit-Hartenberg} correspondentes a cada elo, conforme apresentado no Capítulo 2. A atribuição dos sistemas de coordenadas foi realizada de modo consistente com a configuração 6R e com a interseção dos eixos no punho esférico.
...
\begin{table}[H]
\centering
\caption{Parâmetros de \textit{Denavit-Hartenberg} do manipulador 6R}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagens/3_Metodologia/esquema_eixos.png}
    \caption{Desenho esquemático das juntas e sistemas de referência}
    \label{fig:esquema_eixos}
\end{figure}

\subsection{Modelagem CAD e Exportação}

O modelo mecânico do manipulador foi desenvolvido na plataforma \textit{Onshape}, ferramenta de modelagem CAD baseada em nuvem. Cada elo foi modelado individualmente, respeitando as dimensões adotadas na definição dos parâmetros cinemáticos e priorizando a clareza estrutural do conjunto.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagens/3_Metodologia/OnShape_Assembly.png}
    \caption{Desenho esquemático das juntas e sistemas de refere}
    \caption{Modelo CAD do manipulador serial 6R desenvolvido no \textit{Onshape}.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Imagens/3_Metodologia/render_eixos.png}
...
A implementação da cinemática direta no simulador segue a formulação apresentada no Capítulo 2, na qual o posicionamento do efetuador final é obtida pelo produto sucessivo das matrizes de transformação homogênea associadas a cada elo do manipulador.

No ambiente computacional, essa operação não é implementada explicitamente por meio da multiplicação das matrizes DH. Em vez disso, ela é realizada implicitamente pelo motor gráfico \textit{Babylon.js} por meio da estrutura hierárquica de transformações da cena tridimensional. Nessa estrutura, cada junta do manipulador é representada por um nó de transformação, de modo que as rotações aplicadas às juntas propagam-se automaticamente ao longo da cadeia cinemática, produzindo o mesmo efeito geométrico descrito pelo modelo matemático.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{Imagens/3_Metodologia/slider.png}
...
Para isso, utiliza-se a função \texttt{getWorldMatrix()} do motor gráfico \textit{Babylon.js}, aplicada ao nó correspondente ao efetuador final. Essa função retorna a matriz de transformação acumulada do objeto em relação ao sistema de referência global da cena, incorporando todas as rotações e translações herdadas da cadeia de nós pais.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagens/3_Metodologia/matriz_completa.png}
    \caption{Matriz observada pelo usuário.}
    \label{fig:matriz}
\end{figure}
...
Além do controle direto das variáveis articulares, o simulador disponibiliza um modo alternativo de operação baseado em coordenadas cartesianas no sistema de coordenadas de referência da base \{0\}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{Imagens/3_Metodologia/slider_ci.png}
...
Em termos gerais, singularidades estão associadas à perda de posto da matriz Jacobiana do manipulador. Nessas situações, pequenas variações no posicionamento desejado do efetuador podem resultar em variações muito grandes nas variáveis articulares, ou mesmo tornar impossível a obtenção de uma solução única.
...
Os pontos da trajetória são definidos inicialmente a partir da configuração atual do manipulador. Ao registrar um novo ponto, o sistema armazena os valores das seis variáveis articulares e cria, para cada junta, uma condição de velocidade. Também é armazenado o intervalo de tempo associado ao deslocamento desde o ponto anterior. Consequentemente, o primeiro ponto não possui duração anterior e seu tempo acumulado é igual a zero.

Dessa forma, cada segmento da trajetória é definido por um par de configurações articulares consecutivas, pelas velocidades nas duas extremidades e pelo intervalo de tempo necessário para a transição entre elas. As posições, velocidades e durações podem ser posteriormente ajustadas pelo usuário na tabela de programação, sem que seja necessário cadastrar novamente os pontos.

\subsection{Cálculo dos Coeficientes do Polinômio}

Para cada junta do manipulador, os coeficientes do polinômio cúbico são calculados a partir das posições e velocidades inicial e final do segmento e de sua duração. Para um trecho entre os pontos $P_k$ e $P_{k+1}$, com duração $T_k$, são empregados os coeficientes
...
A mesma velocidade $\dot{q}_k$ é utilizada como condição final do trecho anterior e condição inicial do trecho seguinte. Esse compartilhamento garante continuidade de posição e velocidade nos pontos intermediários. As acelerações são obtidas pela segunda derivada dos polinômios e não são impostas como condições de contorno.
...
\subsection{Sugestão das Velocidades Intermediárias}

Ao serem criados, o primeiro e o último ponto recebem velocidade nula como valor inicial, permanecendo editáveis. Quando a inserção de um novo ponto transforma o ponto anterior em intermediário, o sistema calcula uma sugestão de velocidade para cada junta. Considerando

\begin{align}
m_{k-1} &= \frac{q_k-q_{k-1}}{T_{k-1}}, &
m_k &= \frac{q_{k+1}-q_k}{T_k},
\end{align}

se $m_{k-1}m_k\leq0$, indicando parada ou inversão do sentido do movimento, adota-se $\dot{q}_k=0$. Caso as inclinações tenham o mesmo sentido, utiliza-se a média ponderada

\begin{equation}
\dot{q}_k=
\frac{T_km_{k-1}+T_{k-1}m_k}{T_{k-1}+T_k}.
\end{equation}

A sugestão é realizada uma única vez. Uma velocidade alterada manualmente pelo usuário passa a ser identificada como manual e não é sobrescrita automaticamente pela inserção de novos pontos.
...
A interface principal do simulador é dividida em duas regiões principais. A região superior é destinada à visualização tridimensional do manipulador, enquanto a região inferior concentra os elementos auxiliares de interação e análise, organizados nas abas de gráficos, programação, instruções e dados.
...
No modo de controle cartesiano, o usuário pode definir diretamente o posicionamento desejado do efetuador final. Nesse caso, a interface disponibiliza controles deslizantes para as coordenadas espaciais $x$, $y$ e $z$, bem como para os ângulos de orientação \textit{yaw}, \textit{pitch} e \textit{roll}. 

Sempre que um desses parâmetros é alterado, o algoritmo de cinemática inversa é executado automaticamente, calculando o conjunto de variáveis articulares correspondente ao posicionamento especificado . Os valores resultantes são então aplicados ao manipulador, atualizando imediatamente sua configuração.
...
A interface também permite a definição de trajetórias por meio do registro sequencial de pontos. O usuário pode posicionar o manipulador em uma configuração desejada e armazená-la como parte de uma rotina de movimento. Na aba \textit{Programação}, seis seletores permitem escolher a junta cujos dados serão apresentados. Cada linha da tabela corresponde a um ponto e contém a duração do trecho anterior, o tempo acumulado, a posição angular e a velocidade de contorno.

Os valores de duração, posição e velocidade podem ser alterados diretamente na tabela. Ao selecionar uma linha, o manipulador assume a configuração correspondente e o marcador cartesiano do ponto é destacado, fornecendo uma pré-visualização da condição programada. Durante a execução da rotina, os campos da tabela e os comandos de adição e limpeza permanecem bloqueados para evitar modificações concorrentes nos dados da trajetória.

\begin{figure}[H]
    \centering
    \includegraphics[width=1 \textwidth]{Imagens/3_Metodologia/janela_programacao.png}
    \caption{}
    \label{fig:programacao_trajetoria}
\end{figure}
...
A visualização simultânea dessas grandezas permite acompanhar não apenas a evolução angular das juntas, mas também verificar a continuidade da posição e da velocidade nos pontos intermediários. O gráfico de aceleração permite ainda observar as possíveis descontinuidades dessa grandeza entre segmentos cúbicos consecutivos.
